...
def validate_passports(passports, req_fields, allowed_fields):
    ...
    for passport in passports:
        if set(req_fields).issubset(passport.keys()):  # case A: where all fields are present
            if validate_fields(passport):
                valid_count += 1
        else:  # case B: where some keys are missing
            missing_filds = set(req_fields) - passport.keys()
            if missing_filds.issubset(allowed_fields):  # case C: where only field 'cid' is missing
                if validate_fields(passport):
                    valid_count += 1
    return valid_count

input_passports = parse_passports(data_file)
print(f'Valid Passwords: {validate_passports(input_passports, required_fields, allowed_missing_fields)}')